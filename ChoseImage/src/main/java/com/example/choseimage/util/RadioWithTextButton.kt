...
class RadioWithTextButton @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        isSelected {
            canvas.drawCircle(
                (width / 2).toFloat(), (height / 2).toFloat(), (width / 3).toFloat(), circlePaint
            )
        }
        with(radioType) {
            isRadioText {
                drawTextCentered(
                    canvas, textPaint, text, (width / 2).toFloat(), (height / 2).toFloat()
                )
            } isRadioDrawable {
    ...
    private fun fetchAccentColor(): Int {
        ...
        val a = context.obtainStyledAttributes(typedValue.data, intArrayOf(Color.BLACK))
